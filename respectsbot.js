  ...
  if(!Exists(status.ignore['channel'],e.message.channel.id)){
    ...
    if(exists("channels",e.message.channel.id)==false){
      ...
      if(msg[0] == config.prefix + "help" || msg[0] == config.prefix + "cmd"){
        e.message.channel.sendMessage("`" + config.prefix + "block | " + config.prefix + "help | " + config.prefix + "cmd | " + config.prefix + "eval | 'f' or 'F' to pay respects.`")
        return
      }
      if(msg[0] == config.prefix + "block"){
        if(msg[1] == "channel"){
